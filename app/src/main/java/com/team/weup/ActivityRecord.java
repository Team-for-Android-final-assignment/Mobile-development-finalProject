			...
			switch (v.getId()) {
				...
				case R.id.btn_record:
					...
					if (isRecording == 0) {
						...
						File dir = new File("/storage/emulated/0/notes/");//创建文件夹
						File file = new File("/storage/emulated/0/notes/", str);//创建文件
						...
						Log.d("ActivityRecordTest", "ClickEvent-btn_record-0  FilePath=" + FilePath);
						// 计时器
						mTimer = new Timer();
						// 将麦克图标设置成不可点击，
				...
				case R.id.bt_save:
					if(isRecording == 1){
						Toast.makeText(ActivityRecord.this,"请先停止录音",Toast.LENGTH_SHORT).show();
					}
					else if(isPlaying == 1){
						Toast.makeText(ActivityRecord.this,"请先停止试听",Toast.LENGTH_SHORT).show();
					}
					else {
