...
package com.team.weup.doubledatepicker;
...
public class DoubleDatePickerDialog extends AlertDialog implements OnClickListener, OnDateChangedListener {
    ...
    public DoubleDatePickerDialog(Context context, int theme, OnDateSetListener callBack, int startYear, int startMonthOfYear,
                                  int startDayOfMonth, int endYear, int endMonthOfYear,int endDayOfMonth ,boolean isDayVisible) {
        super(context, theme);
        mCallBack = callBack;
        Context themeContext = getContext();
        setButton(BUTTON_POSITIVE, "确 定", this);
        ...
        mDatePicker_start = (DatePicker) view.findViewById(R.id.datePickerStart);
        mDatePicker_end = (DatePicker) view.findViewById(R.id.datePickerEnd);
        mDatePicker_start.init(startYear, startMonthOfYear, startDayOfMonth, this);
        mDatePicker_end.init(endYear, endMonthOfYear, endDayOfMonth, this);
        // 如果要隐藏当前日期，则使用下面方法。
        if (!isDayVisible) {
            hidDay(mDatePicker_start);
            hidDay(mDatePicker_end);
